function m = MakeExtMap(cmIdqDs, vIds, vIqs, aDats)
	...
	mIdqAB0s = cat(5, mIdA0s, mIqA0s, mIdB0s, mIqB0s);
	mIdqABs = permute(mIdqAB0s, [5,1:4]);

	m1 = mIdqABs(1:4,:)-vIdqABs';
	...
	% fn1s = any(abs(dIdqABs(:, 1:2))>1.e-5, 2) & all(abs(dIdqABs(:, 3:4)) < 1.e-5, 2);
	...
	m = struct('map',mDat, 'axis',{vIds, vIqs}, 'axm', mIdqABs, ...
				'dat', dIdqABs, 'axd', dIdqABs, 'flg', fds);
	...
	fmb = true(size(tmb, 1), 1);
	while any(fmb)
		siztbv = tmb(fmb, :); 
		tcB = tbv(1, idPabD{2});
		fb = all(abs(tmb(:, idPabD{2}) - tcB) < 1.e-5, 2);
		break;
	end
...
function [tMapData, fds] = SetMapData(vIds, vIqs, mIdqABs, tMapData, ...
	...
	if all(abs(ta - tb) < eps, 2)
		...
		% fvs0 = ismembertol(mIdqABs(1:4,:)', vIdqs(1:4,:)', 1.e-5, 'ByRows', true);
		% 	% 結果の検証
		% if sum(fvs0) ~= numel(tIds)	% 補間map Index が出力map Index に存在しない
		...
		% 	for i1 = 1:size(tMapData,2)
			% 	for i2 = 1:size(tMapData,3)
				% 	whos tMapData
				% 	fprintf('---%4d, %4d, %4d\n', i0, i1,i2);
				% 	tMapData(i0, i1, i2, fvs)=shiftdim(mdat(:),-3);
				% 	disp(size(tMapData))
			% 	end
		% 	end
		% end
	...
	% 対象データインデックス
		% 出力マップ Idq Index と 補間map Idq Index の紐付
	% fvs0 = ismembertol(mIdqABs(1:4,:)', vIdqs(1:4,:)', eps, 'ByRows', true);
