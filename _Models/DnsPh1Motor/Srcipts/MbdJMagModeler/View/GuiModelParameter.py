...
"""
    ...
    def _iniViewParam(self) -> QHBoxLayout:
        ...
        e1.editingFinished.connect(
            lambda: self.onValueEdited("maxIa", e1.text())
        )
        ...
        self._model.onMaxIdqChanged.connect(
            lambda v: self.onMaxIdqChanged(t1c, v)
        )
        ...
        e2a.editingFinished.connect(
            lambda: self.onValueEdited("nParaCoil", e2a.text())
        )
        ...
        e3.setAlignment(pn0)
        e3.setFixedWidth(60)
        e3.editingFinished.connect(
            lambda: self.onValueEdited("RaM", e3.text())
        )
        ...
        e4.setAlignment(pn0)
        e4.setFixedWidth(50)
        e4.editingFinished.connect(
            lambda: self.onValueEdited("tmpAtRa", e4.text())
        )
        ...
        e5.setAlignment(pn0)
        e5.setFixedWidth(50)
        e5.editingFinished.connect(
            lambda: self.onValueEdited("mdlNrpm", e5.text())
        )
    ...
    def _loadSettings(self, s: QSettings = None) -> None:
        if s is None:
            s = self._settings
        if s is None:
            return
        s.beginGroup("ModelParameter")
        for pn, typ in self._prmTypes.items():
            if typ is not None:
                v = s.value(pn, defaultValue=None, type=typ)
                setattr(self._model, pn, v)
                print(f"***** Load {pn} = {v}")
        s.endGroup()
    ...
    def SaveSettings(self, s: QSettings = None) -> None:
        ...
        if s is None:
            return
        s.beginGroup("ModelParameter")
        for pn in self._prmTypes:
            s.setValue(pn, getattr(m, pn))
            print(f"***** Save {pn} = {getattr(m, pn)}")
        s.endGroup()

    def __init__(
        self, m: MdlParameter = None, set: QSettings | None = None, parent=None
    ) -> None:
        ...
        self._checkList = {}
        set0 = QSettings(
            QSettings.IniFormat, QSettings.UserScope, "GuiMotorMbd", "Settings"
        )
        ...
        self._loadSettings(self._settings)
        self._model.update()
