...
class WinJMagAnlPrjGen(QWidget):
    ...
    def __init__(
        ...
        acb.stateChanged.connect(
            lambda s: mdl.setIsDoJob(s == Qt.CheckState.Checked.value)
        )
        mdl.onChangedIsDoJob.connect(lambda s: acb.setChecked(s))
        acb.setChecked(mdl.isDoJob)
        ...
        self.onShowErrMsg.connect(
            lambda msg, tit: showDialog(SttDialog.Error, msg, tit, p=self)
        )
    ...
    def generator(self, btn: QPushButton) -> None:
        ...
        if bPath != self._lastPathBasePrj or not os.path.isdir(
            self._lastDirPrjOut
        ):
        ...
        dlg = QProgressDialog(
            "Create JMag Analysis Project File ...", "Cancel", 0, n, self
        )
        ...
        def prgCancel() -> None:
            ...
            self.nmDlg = showDialog(
                SttDialog.NoModeMsg, "Cancel中です.", p=dlg
            )
            self.nmDlg.show()
        ...
        def wrkPrc(evt: threading.Event) -> None:
            try:
                ...
                tLists = self._mdl.genAnlPrjList(
                    tgt, self._dMdl._mdlPrm.maxIa, self.onUpdated, evt
                )
                QApplication.processEvents()
                evt.clear()
    ...
    def _createAnlJob(self, btn: QPushButton) -> None:
        if not self._dMdl.isLinked:
            QMessageBox.critical(
                self, "JMag Link Fail", "Conf  irm. or Relink."
            )
            return
        ...
        dlg = QProgressDialog(
            "Create JMag Analysis Job on JMAG App ...", "Cancel", 0, n, self
        )
    ...
    def loadSettings(self) -> None:
        ...
        if len(bStr) > 0:
            ...
            MdlJMagAnlPrjGen.fromDict(bDic, self.parent(), self._mdl)
