...
class ModelJMagMBD(QObject):
    ...
    def __init__(self, s: QSettings = None) -> None:
        ...
        self._mdlPrm.onTempCoilChanged.connect(
            lambda v: self._mdlMgen.tdCoil.setText(str(v))
        )
    ...
    def extractData(
        ...
        ext.DataExtractor.Extract(
            self._mdlWCS.dataList, self._listStdInfo, self, onPrg, onStop
        )
    ...
    def linkApp(self, isNew: bool = False) -> None:
        if self.isAppOK:
            self.onShowMsg.emit(
                SttDialog.Message, "Already linked to JMag-Designer."
            )
            return
    ...
    def genAnlPrjFiles(
        ...
        if num == 0:
            print(
                f"***** No data to Generate all list. {num}"
                f" / {len(self._mdlWCS.dataList)}"
            )
            return
        ...
        enDs = list(
            set(
                [
                    v.StudyInfo
                    for v in self._mdlWCS.dataList
                    if v.status >= WorkStatus.Defined
                ]
            )
        )
        ...
        ii = (
            max(self._listStdInfo, key=lambda x: x.idx).idx + 1
            if len(enDs)
            else 0
        )
        # 解析jprojファイル作成
        ...
        for k, v in tps.items():
            for i, vv in enumerate(v):
                ...
                if ">>>" not in k:
                    ...
                    if len(kks) == 1:
                        npNam = (
                            f"Mbd{idn}_{fNam0}_{kk[0]}_to_{kks[0]}"
                            f"{vv[-1].valACi[0]:.0f}A.jproj"
                        )
                        nsNam = (
                            f"Mbd{idn} {tsName} {kk[0]} to {kks[0]}:"
                            f"{vv[-1].valACi[0]:.1f}[A]"
                        )
                    else:
                        iav = kks[1].split(",")[i]
                        npNam = (
                            f"Mbd{idn}_{fNam0}_{kk[0]}_to_{kks[0]}{iav}A.jproj"
                        )
                        nsNam = f"Mbd{idn} {tsName} {kk[0]} to {kks[0]}:{iav}A"
                ...
                for twc in vv:
                    ...
                    cds = SetData(twc.valDQi[0], twc.valDQi[1])
                    cds.fPath = tPath
                    cds.sName = nsNam
                    ds.append(cds)
                ...
                if app.HasError():
                    ...
                    sInfo = StudyInfo(
                        nsNam, ts.GetUuid(), JMagPrjFileInfo(tPath, bPath), ii
                    )
                    self._listStdInfo.append(sInfo)
                    for i1, twc in enumerate(vv):
    ...
    def addAnalysisJob(
        ...
        for i, p in enumerate(self._listStdInfo):
            ...
            tgts = sorted(
                [
                    w
                    for w in self.wcList
                    if w.status == WorkStatus.Defined and w.StudyInfo == p
                ],
            ...
            if n0 == 0:
                print(
                    "***** No defined WorkCase to set Analysis job :{p.name}"
                )
                fnUpd.emit(0)
                continue
            QApplication.processEvents()
    ...
    def getStudyLists(self) -> dict[int, str]:
        return {
            i: self._app.GetStudy(i).GetName()
            for i in range(self._app.NumStudies())
        }
