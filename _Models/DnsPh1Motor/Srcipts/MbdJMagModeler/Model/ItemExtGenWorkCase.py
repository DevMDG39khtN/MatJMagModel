...
class ItemExtGenWorkCase(QObject):
    ...
    ########################################################
    def _dataList(self) -> list[2][list[float]]:
        """Get data list."""

        def _getAxList(txt: str) -> list[float] | None:
        ...
        axVals = [_getAxList(re.sub(r"\s+", "", t)) for t in self.axDefText]
        if self.type == TypDefExtCase.Both and len(axVals[1]) == 0:
            axVals[1] = axVals[0]
        return axVals
    ...
    def __repr__(self):
        return (
            f"ItemExtGenWorkCase(Type:{self._isValid}{self.type}"
