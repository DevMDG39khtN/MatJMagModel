...
from JMagDatas.WorkCase import MotAxes, WorkCase
...
class MdlWorkCaseStore(QAbstractTableModel):
    ...
    @staticmethod
    def fromDict(
        data: dict[str, str], tgt: MdlWorkCaseStore = None
    ) -> MdlWorkCaseStore:
        ...
        if len(workCases) > 0:
            tgt.beginResetModel()
            tgt._list = [WorkCase.fromDict(wc) for wc in workCases]
            tgt.endResetModel()
    ...
    def addData(self, v: tuple[float, float], pos: int = -1) -> int:
        pMax = len(self._list)
        if pos > pMax or pos < 0:
            pos = len(self._list)
        nd = WorkCase(v, self._mode, idx=pos)
        self.beginInsertRows(QModelIndex(), self.rowCount(), self.rowCount())
        if pos == len(self._list):
            self._list.append(nd)
        else:
            self._list.insert(pos, nd)
            for w in self._list[pos + 1 :]:
                w.index += 1
        self.endInsertRows()
        n = len(self._list)
        self.dataChanged.emit(n, n, [uQRole.DisplayRole])
        return len(self._list)

    def addDataList(self, v: list[tuple[float, float]], pos: int = -1) -> int:
        if pos >= self.rowCount() or pos < 0:
            pos = self.rowCount()
        posE = pos + len(v)
        self.beginResetModel()
        for i, d in enumerate(v):
            self._list.append(WorkCase(d, self._mode, idx=pos + i))
        self.endResetModel()
        self.dataChanged.emit(pos, posE, [uQRole.DisplayRole])
        n = len(self._list)
        return n
    ...
    def addIdqDatas(
        ...
        n = len(self._list)
        wcBases = [
            WorkCase(v, self._mode, idx=n + i) for i, v in enumerate(vBase)
        ]
        self._list.extend(wcBases)
        n = len(self._list)
        wcExts = [
            WorkCase(v, self._mode, isExA=True, idx=n + i)
            ...
            if nc >= 4 and nc <= 5:  # status GUIで設定不可
                ...
                if isUpd:
                    self.dataChanged.emit(
                        idx, idx, [uQRole.DisplayRole, uQRole.EditRole]
                    )
                    return True
