...
"""
    ...
    def loadSettings(self, s: QSettings | None) -> None:
        if s is not None:
            s.beginGroup("MapGenWin")
            self._lastDirPrj = str(s.value("lastDirPrj", "", str))
            self.restoreGeometry(s.value("PosMapWin"))  # type: ignore
            s.endGroup()
    ...
    def saveSettings(self, s: QSettings | None) -> None:
        if s is not None:
            ...
            s.setValue("PosMapWin", self.saveGeometry())
            s.endGroup()
    ...
    def _iniLayout(self) -> None:
        ...
        lg1 = QGridLayout()
        gbm.setLayout(lg1)
        lg1.addWidget(QLabel("トルク [Nm] : "), 0, 0, alf0)
        lg1.addWidget(QLabel("回転数 [rpm] : "), 1, 0, alf0)
        lg1.addWidget(QLabel("DC電圧 [V] : "), 2, 0, alf0)
    ...
    def _saveJsonData(self) -> None:
        ...
        try:
            ...
            msg = f"保存エラー:{em}\n\n{sm}"
            print(f"@@@@@  {msg}")
            QMessageBox.critical(self, "保存エラー", f"ファイルの保存に失敗しました: {msg}")
