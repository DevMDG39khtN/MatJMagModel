...
class JMagPrjFileInfo:
    ...
    @isMultiSlice.setter
    def isMultiSlice(self, isMulti: bool) -> None:
        self._isMultiSlice = isMulti
        return
    ...
    def saveSettings(self, s: QSettings | None) -> None:
        ...
        s.setValue("isMultiSlice", self._isMultiSlice)
    ...
    def loadSettings(self, s: QSettings | None) -> None:
        ...
        if s is None:
            return
        s.beginGroup("JMagPrjFileInfo")
        self._path = str(s.value("path", "", str))
        self._basePrjPath = str(s.value("basePath", "", str))
        self._isMultiSlice = bool(s.value("isMultiSlice", False, bool))
        self._idx = int(str(s.value("index", -1, int)))
        self._size = int(str(s.value("size", 0)))
        self._uuid = str(s.value("uuid", ""))
        s.beginGroup("times")
        t0 = str(s.value("updated", ""))
        tt0 = datetime.fromisoformat(t0) if len(t0) > 0 else datetime.now()
        t1 = str(s.value("accessed", ""))
        tt1 = datetime.fromisoformat(t1) if len(t1) > 0 else datetime.now()
        t2 = str(s.value("created", ""))
    ...
    def toDict(self) -> dict:
        ...
        return {
            ...
            "isMultiSlice": self._isMultiSlice,
            "index": self._idx,
            "size": self._size,
            "uuid": self._uuid,
            "times": {
    ...
    def fromDict(
        ...
        obj._isMultiSlice = data.get("isMultiSlice", False)
        obj._idx = data.get("index", -1)
        obj._size = data["size"]
        obj._uuid = data["uuid"]
        # if "index" in data:
        #     obj._idx = data["index"]
