...
class MdlJMagAnlPrjGen(QObject):
    ...
    def _genAnlPrjListIdIqNum(
        self,
        tgt0: list[WorkCase],
        fnUpd: SignalInstance | None = None,
    ...
    def _genAnlPrjListIaRmsVals(
        ...
        if self.isExSplit:
            ...
            if self.isFwSplit:
                ...
                kns = ["Div"]
                ids = [np.ones(len(tgtNA), dtype=bool)]  # [np.arange(len(tgtNA))]

        if fnUpd is not None:
            fnUpd.emit(2)

        kn = ",".join([str(v) for v in iaList[1:-1]] + ["Fin"])
    ...
    def loadSettings(self, s: QSettings | None = None) -> None:
        ...
        self._isDivPrj = bool(s.value("isDivPrj", False, bool))
        # self._sttDivPrj = DivStatus(s.value("sttDivPrj", DivStatus, int))
        self._sttDivPrj = DivStatus[str(s.value("sttDivPrj", DivStatus.IdIqNum.name, str))]
        self._numDivPrj = int(str(s.value("numDivPrj", 0, int)))
        self._lstDivPrj = []

        n = s.beginReadArray("lstDivPrj")
        for i in range(n):
            s.setArrayIndex(i)
            v = int(str(s.value("val", 0, int)))
            self._lstDivPrj.append(v)
        s.endArray()
        self._isExSplit = bool(s.value("isExSplit", True, bool))
        self._isFwSplit = bool(s.value("isFwSplit", True, bool))
        self._isDoJob = bool(s.value("isDoJob", False, bool))
    ...
    def saveSettings(self, s: QSettings | None = None) -> None:
        ...
        s.beginGroup("JMagPrjGen")
        s.remove("")
        s.setValue("isDivPrj", self._isDivPrj)
        s.setValue("sttDivPrj", self._sttDivPrj.name)
        s.setValue("numDivPrj", self._numDivPrj)
        s.beginWriteArray("lstDivPrj")
        for i, v in enumerate(self._lstDivPrj):
            s.setArrayIndex(i)
            s.setValue("val", v)
        s.endArray()
        s.setValue("isExSplit", self._isExSplit)
        s.setValue("isFwSplit", self._isFwSplit)
        s.endGroup()
